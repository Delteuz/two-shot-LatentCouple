...
__all__ = ['expand_image', 'set_seed', 'escape_prompt', 'calc_context_size', 'compute_token_merge_indices',
           'compute_token_merge_indices_with_tokenizer', 'image_overlay_heat_map', 'plot_overlay_heat_map',
           'plot_mask_heat_map', 'PromptAnalyzer']
...
def compute_token_merge_indices_with_tokenizer(tokenizer, prompt: str, word: str, word_idx: int = None,
                                               limit: int = -1):
    escaped_prompt = escape_prompt(prompt)
    # escaped_prompt = re.sub(r"[_-]", " ", escaped_prompt)
...
class PromptAnalyzer:
    def __init__(self, clip: FrozenCLIPEmbedderWithCustomWordsBase, text: str):
        ...
        # remade_tokens, fixes, multipliers, token_count = self.tokenize_line(text,
        #                                                                     used_custom_terms=self.used_custom_terms,
        #                                                                     hijack_comments=self.hijack_comments)
        batch_chunks, current_token_count = self.clip.tokenize_line(text)
        # chunk_count = len(batch_chunks)
        # batch_chunk = batch_chunks[0]
        # tokens = batch_chunk.tokens
        # multipliers = batch_chunk.multipliers

        self.token_count = current_token_count
        # self.fixes = batch_chunk.fixes
        self.context_size = calc_context_size(current_token_count)
        ...
        for i in range(self.context_size // 77):
            batch_chunk = batch_chunks[i]
            tokens = batch_chunk.tokens
            multipliers = batch_chunk.multipliers
            self.tokens.extend(tokens)
            self.multipliers.extend(multipliers)

    def create(self, text: str):
        return PromptAnalyzer(self.clip, text)

    def tokenize_line(self, line, used_custom_terms, hijack_comments):
        remade_tokens, fixes, multipliers, token_count = self.clip.tokenize_line(line, used_custom_terms,
                                                                                 hijack_comments)
        return remade_tokens, fixes, multipliers, token_count

    def process_text(self, texts):
        batch_multipliers, remade_batch_tokens, used_custom_terms, hijack_comments, hijack_fixes, token_count = self.clip.process_text(
            texts)
