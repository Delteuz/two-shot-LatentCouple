...
class DiffusionHeatMapHooker(AggregateHooker):
    ...
    def compute_global_heat_map(self, prompt_analyzer, prompt, batch_index, time_weights=None, time_idx=None, last_n=None, first_n=None, factors=None):
        ...
        max_scale_index = max([len(x) for x in all_merges])
        for merge_idx in range(len(all_merges)):
            for i in range(max_scale_index - len(all_merges[merge_idx])):
                all_merges[merge_idx].append(torch.zeros_like(all_merges[merge_idx][0]))
...
class UNetCrossAttentionHooker(ObjectHooker[CrossAttention]):
    ...
    def _hooked_attention(hk_self, self, query, key, value, batch_size, sequence_length, dim, use_context: bool = True):
        ...
        for batch_index in range(hidden_states.shape[0] // slice_size):
            ...
            )   # (batch_count * heads, sequence_length, feature channel per head), can also be interpreted as attention map
            ...
            if use_context and hk_self.calledCount % 2 == 1 and attn_slice.shape[-1] == hk_self.context_size:   # skipping cfg first pass
                if factor >= 1:
                    factor //= 1
                    maps = hk_self._up_sample_attn(attn_slice, value, factor)   # (token_length, 1, img_width, img_height)
